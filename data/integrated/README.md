...
| `combined_section_sorted_flat_comments.pkl` | (있을 경우) 댓글 평탄화 등 flat 전처리 입력용 PKL | [`notebooks/01_preprocess/blog_flat_preprocess.ipynb`](../../notebooks/01_preprocess/blog_flat_preprocess.ipynb) 또는 이전 전처리 흐름 | `blog_flat_preprocess.ipynb` — 없으면 `combined_section_sorted.csv`를 사용 |

현재 `blog_flat_preprocess.ipynb`는 `combined_section_sorted_flat_comments.pkl`이 없을 때 `combined_section_sorted.csv`를 자동으로 읽도록 구성되어 있습니다.
